...
public:
  MMA16816SmemLoader(int nPerWarp, int warpsPerTile, ArrayRef<uint32_t> order,
                     ...
                     const Location &loc, DotOperand operand);

  // lane = thread % 32
  // warpOff = (thread/32) % warpsPerTile(0)
  llvm::SmallVector<Value> computeOffsets(Value lane, Value cSwizzleOffset, DotOperand op) {
    if (canUseLdmatrix) {
      if (op == DotOperand::META) return computeMetaLdmatrixOffs(lane, cSwizzleOffset);
  ...
  // Compute the offset to the metadata matrix this thread is mapped to
  SmallVector<Value> computeMetaLdmatrixOffs(Value lane, Value cSwizzleOffset);
  ...
  Value ldMeta(int batch, int a, int b, ArrayRef<Value> ptrs, Type smemTy);
...
Value MMA16816SmemLoader::ldMeta(int batch,
                           int a, int b, // the coordinates
                           ArrayRef<Value> ptrs, // ptrs are pointers to the offsets
                           Type smemTy) {
  assert(batch == 0 && "Only 2D matrices are supported for loading metadata");
  // The coordinates of the 1x64 matrix from which to load the metadata
  int matIdx[3] = {batch, a, b};

  // In which indices of `ptrs` the metadata values for
  // coordinates (a,b) resides
  int ptrIdx = matIdx[order[0]];

  Value stridedOffset =
      mul(i32_val(matIdx[order[1]] * stridedLoadMatOffset * stridedMatShape),
          stridedSmemOffset);
  Value readPtr = gep(ptr_ty(ctx, 3), smemTy, ptrs[ptrIdx], stridedOffset);

  PTXBuilder builder;
  auto resArg = builder.newListOperand(1, "=r");
  auto addrArg = builder.newAddrOperand(readPtr, "r");
  auto ldmatrix = builder.create("ldmatrix.sync.aligned.m8n8.x1")
                      ->o("shared.b16");
  ldmatrix(resArg, addrArg);

  Value res = builder.launch(rewriter, loc, rewriter.getI32Type());
  return res;
}
...
MMA16816SmemLoader::MMA16816SmemLoader(
  ...
  if (operand != DotOperand::META) numPtrs = std::max<int>(numPtrs, 2);
  ...
  stridedLoadMatOffset = (operand != DotOperand::META) ?
...
SmallVector<Value> MMA16816SmemLoader::computeMetaLdmatrixOffs(
    Value lane, Value cSwizzleOffset) {
  Value warpOff = multiDimWarpId[1];

  // We use the m16n8k16 variant of mma.sp; therfore, we only load
  // with the ldmatrix .x1 variant. For this variant, the first
  // 8 threads in the warp must point to the first 8 rows.
  //  Value innerRow = urem(lane, i32_val(8));
  SmallVector<Value> offs(numPtrs);
  for (int rep = 0; rep < numPtrs; rep++) {
    Value outerCol = i32_val(rep);
    Value outerRow = mul(warpOff, i32_val(warpMatOffset));

    Value phase = urem(udiv(i32_val(1), i32_val(perPhase)), i32_val(maxPhase));
    outerCol = add(outerCol, udiv(cSwizzleOffset, i32_val(contiguousMatShape)));
    outerCol = xor_(outerCol, phase);

    Value innerCol = mul(i32_val(8), urem(lane, i32_val(8)));

    Value i = mul(outerRow, i32_val(stridedMatShape));
    Value j = add(innerCol, mul(outerCol, i32_val(contiguousMatShape)));

    offs[rep] = add(j, mul(i, stridedSmemOffset));
  }
...
Value composeValuesToMetadataOperandLayoutStruct(
    const ValueTable &vals, int batch, int n0, int n1,
    const LLVMTypeConverter *typeConverter, Location loc,
    ConversionPatternRewriter &rewriter) {
  std::vector<Value> elems;
...
getLoadMatrixFn(MemDescType descTy, const SharedMemoryObject &smemObj,
                ...
                Value lane, ValueTable &vals, DotOperand operand,
                const LLVMTypeConverter *typeConverter,
  ...
  auto load = [=, &rewriter, &vals](int batch, int a, int b) {
    MMA16816SmemLoader loader(
        ...
        perPhase, maxPhase, elemBytes, rewriter, typeConverter, loc, operand);
    // Offset of a slice within the original tensor in shared memory
    Value cSwizzleOffset = smemObj.getCSwizzleOffset(order[0]);
    SmallVector<Value> offs = loader.computeOffsets(lane, cSwizzleOffset, operand);
    // initialize pointers
    ...
    if (operand == DotOperand::META) {
      auto meta = loader.ldMeta(batch, a, b, ptrs, smemTy);
      vals[{batch, a, b}] = meta;
    } else {
...
Value loadArg(ConversionPatternRewriter &rewriter, Location loc,
              ...
              const LLVMTypeConverter *typeConverter, Value thread, DotOperand operand) {
  if (operand == DotOperand::META) assert(encoding.getOpIdx() == 0 && "Only A sparse is currently supported.");
  auto shapePerCTA = getShapePerCTA(descTy);
  ...
  auto numRep =
      mmaLayout.getMMAv2Rep(shapePerCTA, bitwidth, encoding.getOpIdx(), encoding.getMeta());
  ...
  if (operand == DotOperand::A)
    warpsPerTile = std::min<int>(warpsPerCTA[1], shapePerCTA[1] / 16);
  else if (operand == DotOperand::B)
    warpsPerTile = std::min<int>(warpsPerCTA[2], shapePerCTA[2] / 16);
  else
    /* Keeping the division by 1 for clarity.
     * This is the number of rows per metadata unit*/
    warpsPerTile = std::min<int>(warpsPerCTA[1], shapePerCTA[1] / 1);
  ...
  if (operand == DotOperand::A)
    loadFn = getLoadMatrixFn(
        ...
        vals /*vals*/, operand /*DotOperand*/, typeConverter /* typeConverter */,
    ...
    loadFn = getLoadMatrixFn(
        descTy, smemObj, mmaLayout, warpsPerTile /*warpsPerTile*/, 2 /*kOrder*/,
        kWidth, {1, 1, 64} /*instrShape*/,
        {1, 1, 64} /*matShape*/,
        multiDimWarpId /*multiDimWarpId*/, lane /*laneId*/,
        vals /*vals*/, operand /*DotOperand*/, typeConverter /* typeConverter */,
        rewriter /*rewriter*/, loc /*loc*/);

  // Perform loading.
  int numRepBatch = numRep[0];
  int numRepOuter = operand != DotOperand::B ? numRep[1] : std::max<int>(numRep[2] / 2, 1);
  int numRepK = operand != DotOperand::B ? numRep[2] : numRep[1];
  for (int b = 0; b < numRepBatch; ++b)
    for (int m = 0; m < numRepOuter; ++m)
      for (int k = 0; k < numRepK; ++k)
        loadFn(b,
               m * (operand == DotOperand::META? 1 : 2),
               k * (operand == DotOperand::META? 1 : 2));

  // Format the values to LLVM::Struct to passing to mma codegen.
  if (operand == DotOperand::META)
    return composeValuesToMetadataOperandLayoutStruct(vals, numRepBatch,
                                                      numRepOuter,
...
Attribute getExpandedEncoding(Attribute encoding) {
  ...
  if (auto sharedEncoding = mlir::dyn_cast<SharedEncodingAttr>(encoding)) {
    ...
    auto expandedEncoding = DotOperandEncodingAttr::get(
        ...
        dotOperandEncoding.getKWidth(), dotOperandEncoding.getMeta());
    return expandedEncoding;
  } else
...
Value convertLayout(int opIdx, signed meta, ConversionPatternRewriter &rewriter,
  ...
  if (meta > 0) {
    ...
    return loadArg(rewriter, loc, expandedDescTy, expandedEncoding,
                        expandedSmemObj, typeConverter, thread, DotOperand::META);
  } else if (opIdx == 0) {
    return loadArg(rewriter, loc, expandedDescTy, expandedEncoding,
                   expandedSmemObj, typeConverter, thread, DotOperand::A);
  } else {
    assert(opIdx == 1);
    return loadArg(rewriter, loc, expandedDescTy, expandedEncoding,
                   expandedSmemObj, typeConverter, thread, DotOperand::B);
  }
}
} // namespace SharedToDotOperandMMAv2
