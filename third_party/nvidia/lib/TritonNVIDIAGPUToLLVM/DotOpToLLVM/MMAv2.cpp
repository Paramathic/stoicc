...
ValueTableV2 getValuesFromDotCompressedOperandLayoutStruct(
    const LLVMTypeConverter *typeConverter, Location loc,
    ConversionPatternRewriter &rewriter, Value value, int batch, int n0, int n1,
    RankedTensorType type) {
  ...
  for (auto b = 0; b < batch; ++b)
    for (auto i = 0; i < n0; ++i) {
      for (auto j = 0; j < n1; j++) {
        vals[{b, 2 * i, j}] = elems[offset++];
        vals[{b, 2 * i + 1, j}] = elems[offset++];
      }
    }
  return vals;
}
...
static void callMmaAmpere(PTXBuilder &builder, int b, int m, int n, int k,
                          ...
                          ValueTableV2 &he, bool isAccF16, bool isIntMMA,
                          int sparsitySelector, bool isSparse) {
  auto retArgs =
  ...
  if (isSparse) {
    ...
    auto eArgs = builder.newOperand(he[{b, m / 2, k / 4}], "r");
    auto spSel = builder.newConstantOperand(sparsitySelector);
...
LogicalResult convertDot(const LLVMTypeConverter *typeConverter,
                         ...
                         Value loadedA, Value loadedB, Value loadedC, Value loadedE,
                         DotOp op, DotOpAdaptor adaptor, bool isTuring) {
  ...
  auto ha = getValuesFromDotOperandLayoutStruct(
      typeConverter, loc, rewriter, loadedA, repBatch, repM,
      e? std::max(1, repK/2) : repK, aTensorTy);
  ...
  if (e) {
    ...
    he = getValuesFromDotMetaOperandLayoutStruct(
        typeConverter, loc, rewriter, loadedE, repBatch, repM,
        std::max(1, repK/4), eTensorTy);
  }
...
LogicalResult convertMMA(triton::DotOp op, triton::DotOp::Adaptor adaptor,
  ...
  return convertDot(typeConverter, rewriter, op.getLoc(), op.getA(),
                    op.getB(), op.getC(), op.getD(), op.getE(), adaptor.getA(),
                    adaptor.getB(), loadedC, adaptor.getE(), op, adaptor, isTuring);
}
