...
from ..sparsity.compressed_sparse import CompressedSparse
...
class JITFunction(KernelInterface[T]):
    ...
    def _get_config(self, *args):
        ...
        def is_divisible_by_16(x):
            ...
            elif isinstance(x, CompressedSparse):
                return (x.data.data_ptr() % JITFunction.divisibility == 0) and \
                       (x.metadata.data_ptr() % JITFunction.divisibility == 0)
            if x is None:
                return True
            return False
    ...
    def run(self, *args, grid, warmup, **kwargs):
        ...
        non_constexpr_vals = self.decompose_sparse_vals(non_constexpr_vals)
