...
from ..sparsity import supported_sparsities
...
class pointer_type(dtype):
    ...
    def __init__(self, element_ty: dtype, address_space: int = 1, sparsity_encoding: str = None):
        ...
        self.name = f'pointer<{element_ty}>' if sparsity_encoding is None else f'pointer<{element_ty}, #spare_encoding={sparsity_encoding}>'

    def to_ir(self, builder: ir.builder) -> ir.pointer_type:
        return builder.get_ptr_ty(self.element_ty.to_ir(builder), 1, self.sparsity_encoding if self.is_sparse() else "")
...
class block_type(dtype):
    ...
    def __init__(self, element_ty: dtype, shape: List, sparsity_encoding=None):
        ...
        #self.shape = get_dense_shape(shape, sparsity_encoding)
        self.shape = shape
        self.numel = 1
        for s in self.shape:
            self.numel *= s
