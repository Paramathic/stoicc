...
unsigned DotOperandEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape,
  ...
  if (auto mmaParent = mlir::dyn_cast<MmaEncodingTrait>(getParent())) {
    if (auto nvMMA = mlir::dyn_cast<NvidiaMmaEncodingAttr>(mmaParent))
      return nvMMA.getTotalElemsPerThreadForOperands(shape, eltTy, getKWidth(),
                                                     getOpIdx(), getMeta());
...
SmallVector<int64_t> NvidiaMmaEncodingAttr::getMMAv2Rep(ArrayRef<int64_t> shape,
  ...
  if (meta > 0) {
    ...
    shapePerWarp = {1, 16, 8, 4 * 64 / bitwidth};
...
unsigned NvidiaMmaEncodingAttr::getTotalElemsPerThreadForOperands(
    ArrayRef<int64_t> shape, Type eltTy, int kWidth, int opIdx, signed meta) const {
  if (meta > 0) {
    assert(isAmpere() && "Sparsity is only supported for Ampere architecture.");
    auto rep = getMMAv2Rep(getShapePerCTA(*this, shape),
                           eltTy.getIntOrFloatBitWidth(),
                           opIdx, meta);
    return rep[0] * rep[1] * rep[2];
  }
  return getTotalElemsPerThreadForOperands(shape, eltTy, kWidth, opIdx);
}
SmallVector<unsigned>
