  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    if (versionMajor == 3) {
      ...
      if (isSparseDot(dotOp)) {
        ...
        auto newEEncoding = DotOperandEncodingAttr::get(
            oldEType.getContext(), sparseIndex, newRetType.getEncoding(),
            oldEType.getElementType(), 1);
        auto newEType = RankedTensorType::get(
            oldEType.getShape(), oldEType.getElementType(), newEEncoding);
        e = rewriter.create<ConvertLayoutOp>(e.getLoc(), newEType, e);
      }
    }
    // convert dot instruction
    auto newDot = rewriter.create<DotOp>(dotOp.getLoc(), newRetType, a, b,
                                         newAcc, e, dotOp.getInputPrecision(),
